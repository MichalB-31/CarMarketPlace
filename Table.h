...
public:
	...
	int read_from_table(User &u, char type);        //Odczytanie danych z tabeli - tylko trzeba tutaj podzielic bo raczej nie chcemy wszystkich informacji na raz
	...
	friend int callbackCar(void* data, int argc, char** argv, char** azColName);
	...
	int car_add_row(string ownerID, string make, string model, string year, string mileage, string body, string forSale);
